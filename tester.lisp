...
(defmethod initialize-instance :after ((frame tester) &rest args)
  ...
  (with-slots (chain cursors) frame
     (loop for i from 0 below 2
	   do (setf (aref cursors i)
		    (make-instance 'standard-flexicursor :chain chain)))))
...
(defun display-app (frame pane)
  (let* ((chain (chain frame))
    ...
    (format pane "~%")
    (loop for i from 0 below 2
	  do (format pane (make-string (* 2 (slot-value (aref cursors i)
							'flexichain::index))
			       :initial-element #\space))
...
(define-tester-command (com-insert<* :name t) ((pos 'integer) (object 'symbol))
...
(define-tester-command (com-delete* :name t) ((pos 'integer))
...
(define-tester-command (com-dd< :name t) ((cursor 'integer))
